...
{
    public partial class UC_KglZttChart : UserControl
    {
        public double[] data;
        public string[] state;
        ...
        private void drawChart(WinChartViewer viewer)
        {
            ...
            double[] viewPortDataSeries = (double[])Chart.arraySlice(data, startIndex, noOfPoints);
            layer.addDataSet(viewPortDataSeries, Chart.CColor(CurveColor));

            CurState = (string[])Chart.arraySlice(state, startIndex, noOfPoints);
            //================================================================================
            //配置轴的刻度和标签
            //================================================================================
        ...
        private void trackLineLabel(XYChart c, int mouseX, int mouseY, int Mx)
        {
            ...
            for (int i = 0; i < c.getLayerCount(); ++i)
            {
                ...
                for (int j = 0; j < layer.getDataSetCount(); ++j)
                {
                    ...
                    if (value == "") continue;
                    PlayTxt = PlayTxt + string.Format(" <*font,Color=000000*>{0}:{1}<*/font*>", "状态", CurState[xIndex]);
                }
            }
